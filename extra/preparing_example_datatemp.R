...
groups <- c("national.Forest.birds","nationalalt.suburban")
names(groups) <- c("Forest","Urban")
sp_list <- NULL
for(i in rev(1:length(groups))){
  g <- groups[i]
  gn <- names(groups)[i]
  tmp2 <- socb[which(socb[,g] == "Included in group"),c("species")]
  if(i == 1){
    tmp2 <- tmp2[-which(tmp2 %in% sp_list$species)]
  }
  tmp <- data.frame(species = tmp2,
                    group = gn)
  
  sp_list <- bind_rows(sp_list,tmp)
}


# sp_sell <- data.frame(species = c("Bobolink","Eastern Meadowlark","Savannah Sparrow","American Kestrel",
#                         "Ovenbird","Blue-headed Vireo","Chestnut-sided Warbler","Red-eyed Vireo",
#                         "Red-winged Blackbird","House Finch","House Wren","American Robin","Chipping Sparrow",""),
#                       group = rep(c("Grassland","Forest","Generalist"),each = 4))
...
write.csv(bird,"Bayesian_Stats_Bird_Counts.csv",row.names = FALSE)
...
sp_sell <- data.frame(species = c("Red-eyed Vireo",
...
write.csv(all,"Bayesian_Stats_Bird_Counts.csv",row.names = FALSE)
